...
export class Duration {
  ...
  getTimeDuration() {
    ...
    return Object.create(Duration.prototype, { [DATA]: { value:Object.freeze({ hours, minutes, seconds, milliseconds, microseconds, nanoseconds }) } });
  }
...
export function cast(dl) {
  ...
  Object.assign(data, { hours, minutes, seconds, milliseconds, microseconds, nanoseconds });
  while (data.nanoseconds > 999) {
    data.nanoseconds -= 1000;
    data.microseconds += 1;
  }
  while (data.microseconds > 999) {
    data.microseconds -= 1000;
    data.milliseconds += 1;
  }
  while (data.milliseconds > 999) {
    data.milliseconds -= 1000;
    data.seconds += 1;
  }
  while (data.seconds > 59) {
    data.seconds -= 60;
    data.minutes += 1;
  }
  while (data.minutes > 59) {
    data.minutes -= 60;
    data.hours += 1;
  }
  // console.error('cast: ', data);
  return Object.create(Duration.prototype, { [DATA]: { value: Object.freeze(data) } });
}
