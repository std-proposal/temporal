...
REGEX.TZOFF = /(?:\+?|\-)(?:[0-1][0-9]|2[0-3]):[0-5][0-9]/;
...
export function balanceMinus(data, truncate) {
  if (isNum(data.nanosecond) && isNum(data.microsecond) && (data.nanosecond < 0)) {
    data.microsecond += Math.floor(data.nanosecond / 1000);
    data.nanosecond = 1000 + (data.nanosecond % 1000);
  }
  if (isNum(data.microsecond) && isNum(data.millisecond) && (data.microsecond > 999)) {
    data.millisecond += Math.floor(data.microsecond / 1000);
    data.microsecond = data.microsecond % 1000;
  }
  if (isNum(data.microsecond) && isNum(data.millisecond) && (data.microsecond < 0)) {
    data.millisecond += Math.floor(data.microsecond / 1000);
    data.microsecond = 1000 + (data.microsecond % 1000);
  }
  if (isNum(data.millisecond) && isNum(data.second) && (data.millisecond > 999)) {
    data.second += Math.floor(data.millisecond / 1000);
    data.millisecond = data.millisecond % 1000;
  }
  if (isNum(data.millisecond) && isNum(data.second) && (data.millisecond < 0)) {
    data.second += Math.floor(data.millisecond / 1000);
    data.millisecond = 1000 + (data.millisecond % 1000);
  }
  if (isNum(data.second) && isNum(data.minute) && (data.second > 59)) {
    data.minute += Math.floor(data.second / 60);
    data.second = data.second % 60;
  }
  if (isNum(data.second) && isNum(data.minute) && (data.second < 0)) {
    data.minute += Math.floor(data.second / 1000);
    data.second = 60 + (data.second % 60);
  }
  if (isNum(data.minute) && isNum(data.hour) && (data.minute > 59)) {
    data.hour += Math.floor(data.minute / 60);
    data.minute = data.minute % 60;
  }
  if (isNum(data.minute) && isNum(data.hour) && (data.minute < 0)) {
    data.hour += Math.floor(data.minute / 60);
    data.minute = 60 + (data.minute % 60);
  }
  if (isNum(data.hour) && isNum(data.day) && (data.hour > 23)) {
    data.day += Math.floor(data.hour / 24);
    data.hour = data.hour % 24;
  }
  if (isNum(data.hour) && (data.hour < 0)) {
    if (isNum(data.day)) data.day += Math.floor(data.hour / 24);
  ...
  if (truncate && isNum(data.day) && isNum(data.month)) {
    if (isNum(data.month) && isNum(data.year) && (data.month < 1)) {
      data.year += Math.floor(data.month / 12);
      data.month = 12 + data.month % 12;
    }
    data.day = Math.min(data.day, daysInMonth(isNum(data.year) ? data.year : 1976, data.month));
  }
  while (isNum(data.day) && isNum(data.month) && (data.day > daysInMonth(isNum(data.year) ? data.year : 1976, data.month))) {
    data.day -= daysInMonth(isNum(data.year) ? data.year : 1976, data.month);
    data.month += 1;
    if (data.month > 12) {
      data.year += 1;
  ...
  while (isNum(data.day) && isNum(data.month) && (data.day < 1)) {
    data.month -= 1;
    data.day += daysInMonth(isNum(data.year) ? data.year : 1976, data.month);
  }
  if (isNum(data.month) && isNum(data.year) && (data.month > 12)) {
    data.year += Math.floor(data.month / 12);
    data.month = data.month % 12;
  }
  if (isNum(data.month) && isNum(data.year) && (data.month < 1)) {
...
export function balancePlus(data, truncate) {
  if (isNum(data.year) && isNum(data.month) && (data.month > 12)) {
    data.year += Math.floor(data.month / 12);
    data.month = data.month % 12;
  }
  if (truncate && isNum(data.month) && isNum(data.day)) {
    data.day = Math.min(data.day, daysInMonth(isNum(data.year) ? data.year : 1976, data.month));
  }
...
export function balance(data) {
  if (isNum(data.nanosecond) && isNum(data.microsecond) && (data.nanosecond > 999)) {
    data.microsecond += Math.floor(data.nanosecond / 1000);
  ...
  if (isNum(data.hour) && (data.hour > 23)) {
    data.hour = data.hour % 24;
  }
  while (isNum(data.day) && isNum(data.month) && (data.day > (isNum(data.year) && isLeapYear(data.year) ? 366 : 365))) {
    data.day -= (isNum(data.year) && isLeapYear(data.year) ? 366 : 365);
    if (isNum(data.year)) data.year += 1;
  }
  while (isNum(data.day) && isNum(data.month) && (data.day > daysInMonth(isNum(data.year) ? data.year : 1976, data.month))) {
    data.day -= daysInMonth(isNum(data.year) ? data.year : 1976, data.month);
    data.month += 1;
...
function isNum(v) {
  return (('undefined' !== v) && Number.isInteger(v));
}
