...
export function calculate(
  ...
  microsecond = (microsecond + Math.floor(nanosecond / 1e3));
  nanosecond = Math.floor(nanosecond % 1e3);

  while (microsecond < 0) { microsecond += 1E3; millisecond -= 1; }
  millisecond = (millisecond + Math.floor(microsecond / 1e3));
  microsecond = Math.floor(microsecond % 1e3);

  while (millisecond < 0) { millisecond += 1E3; second -= 1; }
  second = (second + Math.floor(millisecond / 1e3));
  millisecond = Math.floor(millisecond % 1e3);
