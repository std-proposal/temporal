...
export function balance(data, truncate = false) {
  if ((data.month !== undefined) || (data.day !== undefined)) {
    ...
    while (data.day > daysInMonth(data.year, data.month)) {
      ...
      data.month += 1;
      if (data.month > 12) { data.year += 1; data.month -= 12; }
    }
    while (data.day < 1) {
      data.month -= 1;
      if (data.month < 1) { data.year -= 1; data.month += 12; }
      data.day += daysInMonth(data.year, data.month);
    }
  }
  ...
  if ((data.hour !== undefined) || (data.minute !== undefined)) {
    ...
    if (data.hour > 23) {
      if (data.day !== undefined) { data.day += Math.floor(data.hour / 24); }
      data.hour = data.hour % 24;
    }
    if (data.hour < 0) {
      if (data.day !== undefined) { data.day += Math.ceil(data.hour / 24) - 1; }
