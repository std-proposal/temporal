...
export class DateTime {
  ...
  withZone(timeZone, choice = EARLIER) {
    ...
    const dateTime = this;
    const absolutes = timeZone.absolute(dateTime);
    let absolute;
    switch (choice) {
      case EARLIER:
        absolute = absolutes.shift();
        break;
      case LATER:
        absolute = absolutes.pop();
        break;
      default:
        absolute = absolute.find((absolute) => timeZone.offset(absolute) === choice);
        break;
    }
    if (!absolute) throw TypeError('invalid DateTime int TimeZone');
    return absolute
  }
